...
class Query(BaseQuery):
    def soft_delete(self):
        return self.update({"delete_at": datetime.now()})

    @property
    def logic_query(self):
        return self.filter_by(delete_at=None)

    def logic_all(self):
        return self.logic_query.all()

    def all_json(self, schema: Marshmallow().Schema):
        return schema(many=True).dump(self.all())

    def logic_all_json(self, schema: Marshmallow()):
        return schema(many=True).dump(self.logic_all())
